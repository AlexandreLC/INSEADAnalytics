...
For example, cereals are higly correlated with one another (Corn and wheat are correlated with 0.63 correlation coefficient). It does make sense as they look alike (meaning supply and demand are similar and move similarly).
...
1. Using the table showing the cumulative explained variance, we see that we need only 6 principal components to explain the more than 50% of the data (52.29%).
2. "5 yr T-notes US and "10 yr T-notes US" with a weight of 0.93.
3. Factors 1 to 5 are either monetary or stock clusters (by region that usually go together). Factor 6 is Oil. Factor 7 is raw metal. Component 8 is cereals. Component 9 is precious metals (gold, silver, etc...). Component 10 is Asian stock market.
4. This is the pre-crisis rotated principal components:

<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
pre_crisis_data = futures_data[as.Date(rownames(futures_data)) < "2007-10-01" , ]
Variance_Explained_Table_results_pcd<-PCA(pre_crisis_data, graph=FALSE)
Variance_Explained_Table_pcd<-cbind(paste("component",1:ncol(pre_crisis_data),sep=" "),Variance_Explained_Table_results_pcd$eig)
Variance_Explained_Table_pcd<-as.data.frame(Variance_Explained_Table_pcd)
colnames(Variance_Explained_Table_pcd)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
show_data_pcd = data.frame(Variance_Explained_Table_pcd)
m1<-gvisTable(show_data_pcd,options=list(width=1920, height=min(400,27*(nrow(show_data_pcd)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
<br>
```{r Fig1_pcd, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
eigenvalues_pcd  <- Variance_Explained_Table_pcd[,2]
df_pcd           <- cbind(as.data.frame(eigenvalues_pcd), c(1:length(eigenvalues_pcd)), rep(1, length(eigenvalues_pcd)))
colnames(df_pcd) <- c("eigenvalues", "components", "abline")
Line_pcd         <- gvisLineChart(as.data.frame(df_pcd), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_pcd, 'chart')
```
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
corused_pcd = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results_pcd<-principal(corused_pcd, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors_pcd<-round(Rotated_Results_pcd$loadings,2)
Rotated_Factors_pcd<-as.data.frame(unclass(Rotated_Factors_pcd))
colnames(Rotated_Factors_pcd)<-paste("Component",1:ncol(Rotated_Factors_pcd),sep=" ")
sorted_rows_pcd <- sort(Rotated_Factors_pcd[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_pcd <- Rotated_Factors_pcd[sorted_rows_pcd,]
Rotated_Factors_pcd[abs(Rotated_Factors_pcd) < 0.3]<-NA

show_data_pcd <- Rotated_Factors_pcd 
show_data_pcd<-cbind(rownames(show_data_pcd),show_data_pcd)
colnames(show_data_pcd)<-c("Variables",colnames(Rotated_Factors_pcd))
m1<-gvisTable(show_data_pcd,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_pcd)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
<br>
<br>
**And there are the crisis rotated principal components:**
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
crisis_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]
Variance_Explained_Table_results_cd<-PCA(crisis_data, graph=FALSE)
Variance_Explained_Table_cd<-cbind(paste("component",1:ncol(crisis_data),sep=" "),Variance_Explained_Table_results_cd$eig)
Variance_Explained_Table_cd<-as.data.frame(Variance_Explained_Table_cd)
colnames(Variance_Explained_Table_cd)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
show_data_cd = data.frame(Variance_Explained_Table_cd)
m1<-gvisTable(show_data_cd,options=list(width=1920, height=min(400,27*(nrow(show_data_cd)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
<br>
```{r Fig1_cd, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
eigenvalues_cd  <- Variance_Explained_Table_cd[,2]
df_cd           <- cbind(as.data.frame(eigenvalues_cd), c(1:length(eigenvalues_cd)), rep(1, length(eigenvalues_cd)))
colnames(df_cd) <- c("eigenvalues", "components", "abline")
Line_cd         <- gvisLineChart(as.data.frame(df_cd), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_cd, 'chart')
```
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
corused_cd = cor(crisis_data[,apply(crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results_cd<-principal(corused_cd, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors_cd<-round(Rotated_Results_cd$loadings,2)
Rotated_Factors_cd<-as.data.frame(unclass(Rotated_Factors_cd))
colnames(Rotated_Factors_cd)<-paste("Component",1:ncol(Rotated_Factors_cd),sep=" ")

sorted_rows_cd <- sort(Rotated_Factors_cd[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_cd <- Rotated_Factors_cd[sorted_rows,]
Rotated_Factors_cd[abs(Rotated_Factors_cd) < 0.3]<-NA

show_data_cd <- Rotated_Factors_cd 
show_data_cd<-cbind(rownames(show_data_cd),show_data_cd)
colnames(show_data_cd)<-c("Variables",colnames(Rotated_Factors_cd))
m1<-gvisTable(show_data_cd,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_cd)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
<br>


4. There are many differences, but to me the most important is that raw metals (copper, zinc, nickel, ...) are the 4th most important component before the crisis, but only 8th during the crisis. During the  crisis, stock markets and 
<br>
...
1. The code works as follows:

+ The matrix `signal_used` represents the weighted averages of each investment decision. It is initialized as a matrix full of '0' and populated with the sum of all moving averages with a window from 10 to 250 with an increment of 20. 
+ These are normalized so that everyday we invest only 1€.
+ They are then shifted two days backwards so as to simulated real life (a trader would only have access to past data of course)
+ Finally, the `signal_used` matrix is multiplied by the return matrix to simulated the buys and sells of each day.
+ The result is the sum for each day of the profits and losses made omn each item of the portfolio.

2.I will report here two strategies.

The first one is using a very simple strategy to have a very short window of 5 days (i.e. a working week). For the second one, I will use a window of 300 days.

**Results of the first strategy (5 days moving window).**


```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = sign(apply(futures_data,2, function(r) ma(r,5)))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
...
**Results of the second strategy (300 days moving window).**


```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = sign(apply(futures_data,2, function(r) ma(r,300)))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
...
<br>
<br>
It seems that the 5 days moving window is way to short to deliver good results, the 300 days strategy leads significantly better ones.
...
Just for fun, I tried to find the optimal window length. 

```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used_tmp = 0*futures_data # just initialize the trading signal to be 0
MAfreq_tmp<-seq(5,305,by=10)
Returns<-0*1:length(MAfreq_tmp)
names(Returns)<-MAfreq_tmp

#Initializing the variables for the optimization
Best_Returns<-0*1:nrow(futures_data)
Best_window<-MAfreq_tmp[1]
Best_window_index<-1

for (iter in 1:length(MAfreq_tmp)){
  signal_used_tmp = sign(apply(futures_data,2, function(r) ma(r,MAfreq_tmp[iter])))
  signal_used_tmp = t(apply(signal_used_tmp,1,function(r) {
    res = r  
    if ( sum(abs(r)) !=0 )
      res = r/sum(abs(r))
    res
  ...
  # Now create the returns of the strategy for each futures time series
  strategy_by_future_tmp <- scrub(shift(signal_used_tmp,2)*futures_data) # use the signal from 2 days ago
  # finally, this is our futures trend following strategy
  trading_strategy_tmp = apply(strategy_by_future_tmp,1,sum)
  # Recording the results of the returns with this particular window
  final_results=1
  for (i in 1:length(trading_strategy_tmp))
    final_results=final_results*(1+trading_strategy_tmp[i])
  
  #Recording the strategy that maximises our returns
  if (final_results>max(Returns)){
    Best_Returns=trading_strategy_tmp
    Best_window=MAfreq_tmp[iter]
    Best_window_index=iter
  }
  
  # Recording the results of that particular computation
  Returns[iter]=final_results
}
names(Best_Returns) <- rownames(futures_data)
```

<br>

A piece of code gives me the returns with respect to different values of window length. In the table below the results are displayed.

<br>

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(cbind(names(Returns), Returns))
m1<-gvisTable(show_data,options=list(width=1220, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE))
print(m1,'chart')
```
...
Below is the graph corresponding to the best return possible among the different window lengths tested. The optimizing window length her is `r Best_window`, and the corresponding return is `r Returns[Best_window_index]`. The corresponding plot over time is as follows.
<br>

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=5,fig.width= 8, fig=TRUE}
pnl_plot(Best_Returns)
```

It yields however inferior results when compared to the example provided above. And nothing indicates anyway that this strategy would work in the future...
