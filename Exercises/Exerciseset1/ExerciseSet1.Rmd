...
1. In the *dataSet1.R* file the variable *mytickers* is initialized with the data to retrieve, here the S&P500 and Apple :  *mytickers = c("SPY", "AAPL")*.
<br>
Then the *for* loop downloads the data related to *SPY* and *AAPL*, in the line:
<br>
*tmpdata<-as.matrix(try(getSymbols(Symbols=ticker_to_get,from = startDate,auto.assign=FALSE)))*. 
<br>
<br>

2. Here are the statistics for the Apple stock:

* The cumulative returns of the Apple stock during this period is **`r round(100*sum(StockReturns[,2]),1)`%**.
* The average daily returns of the Apple stock during this period is **`r round(100*mean(StockReturns[,2]),3)`%**;
* The standard deviation of the daily returns of the Apple stock during this period is **`r round(100*sd(StockReturns[,2]),3)`%**;
<br>
<br>

3. To add *Yahoo* data, we need to modify the script *dataSet1.R* a bit, namely adding "YHOO" to the indices to retrieve. This is performed by modifying the variable *mytickers*, currently initialized as *c("SPY", "AAPL")* by adding Yahoo! to the list. The code should show *mytickers = c("SPY", "AAPL", "YHOO")*.
<br>
<br>
Then we can compute the statistics in the same way as before:

* The cumulative returns of the Yahoo! stock during this period is **`r round(100*sum(StockReturns[,3]),1)`%**.
* The average daily returns of the Yahoo! stock during this period is **`r round(100*mean(StockReturns[,3]),3)`%**;
* The standard deviation of the daily returns of the Yahoo! stock during this period is **`r round(100*sd(StockReturns[,3]),3)`%**;
<hr>
<hr>
...
Following the Extra Question in Part I, the variable *StockReturns* also contains data for Yahoo, and following the Extra Question in Part III it also contains data for Microsoft. This is therefore reflected in the number of rows contained in *transposedData*. <br> 
...
2. Here we just have to apply the function *mean* on columns instead of rows (as well as adjust for the names). To account for the additional Yahoo and Microsoft data, I twitched the code a little bit.
<br>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
...
1. The start Date is initilized by the command "startDate = "2001-01-01"". It's already set to 2001-01-01.
<br>
<br>

2. I chose to do a portfolio with equal weight on Microsoft (MSFT) and Apple (AAPL). Therefore, I added "MSFT" to the variable *mytickers*
<br>

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
DataToPlot <- matrix(c(StockReturns[,"AAPL"],StockReturns[,"MSFT"]), nrow = length(StockReturns[,"AAPL"]), ncol=2, byrow=FALSE)
portfolio = apply(DataToPlot,1,mean)
names(portfolio) <- rownames(StockReturns)
pnl_plot(portfolio)
```
<br>
...
<br>
<br>
1. The command `sum(myData != StockReturns[1:20,])` sums the results of the logical equation determining if the variable my Data is the same (component per component) as what has been stored and then read in the CSV file. If the write/read had been correctly done, the command should return 0, saying that the two variables are identical. 
<br>
However, the *read* function used the separator ";" instead of ",". Therefore, the equation returns `nrows(myData)*ncol(myData)`, thus saying that the two variables are totally different. Indeed, the csv file was incorrectly read and, not finding the separator ";", concatenated all the data amond each line.
<br>
<br>
2. The command `myData + StockReturns[1:40,]` is supposed to return a matrix corresponding to the sum of both matrices. However, the data present in *myData* in not numeric (it's a string of characters corresponding to the concatenation of all the data on each line). Therefore RStudio returns an error. 
<hr>
<hr>
